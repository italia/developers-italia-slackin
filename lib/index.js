  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      dom('head',
        ...
        dom('link rel=stylesheet href=https://fonts.googleapis.com/css?family=Titillium+Web:400,700'),
        dom(`link rel=stylesheet href=${path}assets/developers.css`),
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'Canale non autorizzato' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Canale non trovato "${channel}"` })
      }
    }
    ...
    if (!email) {
      return res
      .status(400)
      .json({ msg: 'Nessuna email inserita' })
    }

    if(captcha_response == undefined || !captcha_response.length){
      return res
      .status(400)
      .send({ msg: 'Captcha non corretto' });
    }

    if (!remail().test(email)) {
      return res
      .status(400)
      .json({ msg: 'Email non corretta' })
    }
    ...
    if (emails && emails.indexOf(email) === -1) {
      return res
      .status(400)
      .json({ msg: 'La tua email non è nella lista' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'L\'accettazione della CoC è obbligatoria' })
    }
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          invite({ token, org, email, channel: chanId }, err => {
            if (err) {
              if (err.message === `Apertura Slack in corso...`) {
                return res
                .status(303)
            ...
            res
            .status(200)
            .json({ msg: 'Controlla la tua email!' })
          });
          ...
          if (err) {
            return res
            .status(400)
            .send({ msg: 'Il controllo del Captcha non è andato a buon fine' });
          }
        }
